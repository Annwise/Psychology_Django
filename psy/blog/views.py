...
def blog(request):
    # Получаем кешированные статьи
    articles = cache.get('blog_articles')
    ...
    if articles is None:
        ...
        cache.set('blog_articles', articles, 300)  # кешируем на 5 минут

    # Получаем параметры
    sort = request.GET.get('sort', '-published_date')  # новые - первыми
